...
2015년 6월에 공개된 새로운 자바스크립트(JS) 표준 ECMAScript 6는 새로운 기능이 많이 추가되었으며,<br>
2009년 이후의 첫번째 업데이트입니다.
(앞으로는 ES2015 처럼 발표년도를 뒤에 붙여서 읽는다고 합니다.)

아직 주요 브라우저들의 자바스크립트 엔진에서 이를 완벽히 지원하진 않지만 향상된 기능을 경험해 볼 수 있습니다.<br>
주요 브라우저에서 ECMAScript를 사용하기 위한 방법인 Transpiler 와 개념, 문법에 대해 알아 보도록 하겠습니다.

## ECMAScript가 도데체 몬데?

ECMA스크립트(ECMAScript)는 Ecma 인터내셔널의 ECMA-262 기술 규격에 정의된 **표준화를 위한 자바스크립트 기술 규격** 입니다.<br>

  표준화 차세대 JavaScript의 사양이다 라고 생각하시면 됩니다.
...
ES2015 에서 블록 스코프 안에서 변수를 대체 선언하는 용도로 `let`, `const` 키워드가 추가 되었습니다.

자바스크립트는 함수 내의 블록내(if문이나 for문 등)에 선언되어도 변수의 스코프가 함수 전체로 되어버리는데,<br>
**var대신 let이라는 키워드** 를 사용하여 일반 자바나 c#처럼 블록 내 에서만 사용할 수 있는 변수 가 되며,<br>
...
weakly하게 값을 참조한다는 뜻인데 WeakSet이 갖는 객체의 참조값이 다른곳에서 참조되지 않으면 객체는 garbage collect 대상이 됩니다.<br>
...
WeakSet 과 비슷한 개념으WeakMap 의 key 가 약하게 참조됩니다.<br>
...
마지막으로
transpiler는 compiler와 같이 코드를 무언가로 변환하기 위하여 사용 합니다.<br>
compiler는 코드를 바이트 코드로 변환하지만, transpiler는 코드를 같은 레벨의 다른 언어로 변환합니다.

JS transpiler는 coffeescript, typescript, babel, traceur 가 있습니다.<br>
...
앞서 이야기한 바와 같이 현재 ES6 스펙을 100% 지원하는 브라우저는 없습니다.<br>
...
어 코드는 **방어 코드를 간결히 해주는 주요 문법** 중 하나입니다.

하지만! 현재 거의 모든 브라우저가 이 문법을 **지원하지 않습니다.** <br>
(실행시 Syntax Error 를 발생합니다.)<br>
IE11 에서는 함수 중첩의 복잡함을 해소할 수 있는 Arrow function 스펙을 지원하지 않고 있습니다.
...
babel 또는 traceur transpiler를 사용하면 ES2015 코드들을 전부 ES5 코드로 변환해 사용 가능하기 때문에<br>
현재까지 출시된 모든 '모던' 브라우저에서 문제없이 동작이 가능 합니다.<br>
...
듣기만 해도 복잡하고 어려울 것 같지만<br>
변환과 압축을 한방에 해주는 도구들이 이미 많이 배포가 되어 있는 상황이기 때문에 설정 파일을 만들고 명령어를 실행만 하면 됩니다.<br>
...
추가적으로 Unicode, Modules, Module Loaders,  Destructuring, Proxies, Symbols등등 새롭게 추가 되거나 변경 확장된 문법들이 많습니다. <br>
이런 문법들에 대해서 사용 방법이 어떤건지 궁금해 지고 사용해 보고 싶어지 셨다면....
저의 이 시간은 대 성공 이군요~

.

모두 다 같이 두려워 하지 말고 새로운 표준화 차세대 JavaScript로 함께 여행을 떠나시죠^^
